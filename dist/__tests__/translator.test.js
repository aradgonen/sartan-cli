    ...
    managers.forEach(manager => {
        describe(`with ${manager}`, () => {
            it('should handle simple install', () => {
                const { finalCommand, finalArgs } = (0, translator_1.translateCommand)(manager, 'install', []);
                expect(finalCommand).toBe('install');
                expect(finalArgs).toEqual([]);
            });
            it('should handle adding a package', () => {
                const { finalCommand, finalArgs } = (0, translator_1.translateCommand)(manager, 'install', ['react']);
                if (manager === 'npm') {
                    expect(finalCommand).toBe('install');
            ...
            it('should handle uninstalling a package', () => {
                const { finalCommand, finalArgs } = (0, translator_1.translateCommand)(manager, 'uninstall', ['react']);
                if (manager === 'npm') {
            ...
            it('should handle running a script', () => {
                const { finalCommand, finalArgs } = (0, translator_1.translateCommand)(manager, 'run', ['dev']);
                if (manager === 'yarn') {
            ...
            it('should handle pass-through commands', () => {
                const { finalCommand, finalArgs } = (0, translator_1.translateCommand)(manager, 'test', ['--watch']);
